...
3. **分析代码库/查重复/死代码/架构违规** → 加载 `codebase-analysis` 插件
...
当用户提出以下意图时,请自动调用对应的技能:

1. **用户给出的需求分析/需求解构前** -> 优先加载 `requirement-analyzer` 技能
2. **所有测试用例/测试脚本的生成/执行前** -> 优先加载 `test-automation` 技能
